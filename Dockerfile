FROM ubuntu
RUN apt -qq update && apt -qq -y dist-upgrade && DEBIAN_FRONTEND="noninteractive" TZ="UTC" apt --no-install-recommends -qq -y install ca-certificates curl unzip texlive-xetex ghostscript python3
